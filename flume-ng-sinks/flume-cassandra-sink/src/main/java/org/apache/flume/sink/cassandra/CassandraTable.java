  ...
  public Map<String, Object> parse(final Event event) {
    ...
    for (final ColumnMetadata column : columns) {
      ...
      if (headers.containsKey(columnName) && !columnName.equals(bodyColumn)) {
        result.put(columnName, CassandraUtils.parseValue(column.getType(), headers.get(columnName)));
      } else if (columnName.equals(bodyColumn)) {
        result.put(columnName, CassandraUtils.parseValue(column.getType(), new String(event.getBody(), Charsets.UTF_8)));
      }
    }
